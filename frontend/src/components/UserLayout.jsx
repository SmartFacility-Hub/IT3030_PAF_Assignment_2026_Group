...
const themes = {
  dark: {
    ...
    "--status-green": "#4ade80", "--status-red": "#f87171",
    "--status-blue": "#60a5fa", "--status-amber": "#fbbf24",
    "--status-teal": "#2dd4bf", "--status-teal-bg": "rgba(45,212,191,0.15)",
  ...
  light: {
    ...
    "--status-green": "#15803d", "--status-red": "#dc2626",
    "--status-blue": "#2563eb", "--status-amber": "#d97706",
    "--status-teal": "#0d9488", "--status-teal-bg": "rgba(13,148,136,0.12)",
...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sidebar-role-badge {
    ...
    background: var(--status-teal-bg); border: 1px solid rgba(45,212,191,0.3);
    border-radius: var(--radius-sm); font-family: var(--font-mono);
    font-size: 10px; font-weight: 500; color: var(--status-teal);
    letter-spacing: 0.1em; text-transform: uppercase;
    display: flex; align-items: center; gap: 6px;
  }
  .role-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--status-teal); animation: pulse 2s infinite; }
  ...
  .nav-item.active {
    ...
    border-color: rgba(45,212,191,0.3); font-weight: 600;
  }
  ...
  .nav-badge.red { background: var(--status-red); }
  ...
  .profile-avatar {
    width: 34px; height: 34px; border-radius: 50%; background: var(--status-teal);
  ...
  .topbar-menu-btn:hover { border-color: rgba(45,212,191,0.4); color: var(--status-teal); }
  .topbar-breadcrumb {
  ...
  .topbar-search:hover { border-color: rgba(45,212,191,0.4); }
  .search-kbd {
  ...
  .icon-btn:hover { border-color: rgba(45,212,191,0.4); color: var(--status-teal); background: var(--status-teal-bg); }
  .notif-dot {
  ...
  .user-page-content.sidebar-collapsed { margin-left: 0; }
  ...
  @media (max-width: 768px) {
    ...
    .user-page-content { margin-left: 0 !important; }
  }
`;

const breadcrumbs = {
  "/dashboard":            { title: "My Dashboard",   sub: "Overview" },
  "/dashboard/bookings":   { title: "My Bookings",    sub: "Reservations" },
  "/dashboard/incidents":  { title: "My Incidents",   sub: "Reports" },
  "/dashboard/browse":     { title: "Browse Rooms",   sub: "Facilities" },
};
...
const navSections = [
  {
    label: "My Space",
    items: [
      { icon: "📊", label: "Dashboard",     id: "dashboard",  route: "/dashboard" },
      { icon: "🔔", label: "Notifications", id: "notifs",     route: "/dashboard/notifications", badge: "2" },
    ],
  },
  {
    label: "Bookings",
...
export default function UserLayout() {
  ...
  const crumb = breadcrumbs[location.pathname] || { title: "My Dashboard", sub: "Overview" };
  ...
  const getRoleLabel = () => {
    ...
    if (roles.includes("ROLE_LECTURER")) return "Faculty Portal";
    if (roles.includes("ROLE_STUDENT"))  return "Student Portal";
    return "User Portal";
  };
  ...
  return (
    <>
      ...
      {/* ── PAGE CONTENT ── */}
      <div className={`user-page-content${sidebarOpen ? "" : " sidebar-collapsed"}`}>
        <Outlet />
      </div>
    </>
  );
