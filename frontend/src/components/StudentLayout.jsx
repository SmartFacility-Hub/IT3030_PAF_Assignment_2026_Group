...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sidebar-role-badge {
    ...
    background: var(--status-blue-bg); border: 1px solid rgba(96,165,250,0.3);
    border-radius: var(--radius-sm); font-family: var(--font-mono);
    font-size: 10px; font-weight: 500; color: var(--status-blue);
    letter-spacing: 0.1em; text-transform: uppercase;
    display: flex; align-items: center; gap: 6px;
  }
  .role-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--status-blue); animation: pulse 2s infinite; }
  ...
  .nav-item.active { background: var(--status-blue-bg); color: var(--status-blue); border-color: rgba(96,165,250,0.3); font-weight: 600; }
  ...
  .profile-avatar {
    width: 34px; height: 34px; border-radius: 50%; background: var(--status-blue);
  ...
  .student-page-content.sidebar-collapsed { margin-left: 0; }
  ...
  @media (max-width: 768px) {
    ...
    .student-page-content { margin-left: 0 !important; }
  }
`;
...
const themes = {
  dark: {
    ...
    "--status-amber": "#fbbf24", "--status-green-bg": "rgba(74,222,128,0.15)",
    "--status-red-bg": "rgba(248,113,113,0.15)", "--status-blue-bg": "rgba(96,165,250,0.15)",
    "--status-amber-bg": "rgba(245,166,35,0.15)", "--shadow-card": "0 4px 24px rgba(0,0,0,0.3)",
  },
  light: {
    ...
    "--status-amber": "#d97706", "--status-green-bg": "rgba(21,128,61,0.10)",
    "--status-red-bg": "rgba(220,38,38,0.10)", "--status-blue-bg": "rgba(37,99,235,0.10)",
    "--status-amber-bg": "rgba(212,132,10,0.12)", "--shadow-card": "0 2px 12px rgba(0,0,0,0.08)",
  },
};
...
const navSections = [
  {
    label: "My Space",
    items: [
      { icon: "📊", label: "Dashboard",     id: "dashboard", route: "/student" },
      { icon: "🔔", label: "Notifications", id: "notifs",    route: "/student/notifications", badge: "2" },
    ],
  },
  {
    label: "Bookings",
  ...
  },
  {
    label: "Support",
    items: [
      { icon: "🔧", label: "My Incidents",  id: "incidents", route: "/student/incidents" },
      { icon: "📝", label: "Report Issue",  id: "report",    route: "/student/report" },
    ],
  },
];
...
export default function StudentLayout() {
  ...
  const crumb = breadcrumbs[location.pathname] || { title: "My Dashboard", sub: "Student" };
  ...
  return (
    <>
      ...
      <aside className={`sidebar${sidebarOpen ? "" : " collapsed"}`}>
        <a className="sidebar-logo" href="/student">
      ...
      {/* ── PAGE CONTENT ── */}
      <div className={`student-page-content${sidebarOpen ? "" : " sidebar-collapsed"}`}>
        <Outlet />
      </div>
    </>
  );
