...
const styles = `
  ...
  .page-label::before { content: ''; display: block; width: 18px; height: 1px; background: var(--status-blue); }
  ...
  .welcome-banner {
    ...
    border: 1px solid var(--border); border-left: 4px solid var(--status-blue);
  ...
  .welcome-banner::before {
    ...
    background: var(--status-blue-bg); pointer-events: none;
  }
  .welcome-avatar {
  ...
  .kpi-card::before { content: ''; position: absolute; inset: 0; background: var(--status-blue-bg); opacity: 0; transition: opacity 0.3s; }
  ...
  .card-action { font-family: var(--font-mono); font-size: 11px; color: var(--status-blue); cursor: pointer; transition: color 0.2s; display: flex; align-items: center; gap: 4px; border: none; background: none; padding: 0; }
  ...
  .form-input:focus, .form-select:focus, .form-textarea:focus { border-color: rgba(96,165,250,0.5); }
  ...
  @media (max-width: 768px)  { .kpi-grid { grid-template-columns: 1fr 1fr; } .content-grid-equal { grid-template-columns: 1fr; } .calendar-strip { grid-template-columns: repeat(4,1fr); } .room-grid { grid-template-columns: 1fr; } }
`;
...
const notifications = [
  { icon: "✅", bg: "var(--status-green-bg)", text: <><strong>Booking Approved</strong> — Computer Lab 2 on Tue 8 Apr approved</>,                                    time: "5 min ago",  unread: true  },
  { icon: "💬", bg: "var(--status-blue-bg)",  text: <><strong>New Comment</strong> — Technician added update to ticket <strong>#INC-091</strong></>,                   time: "1 hr ago",   unread: true  },
  { icon: "⏳", bg: "var(--status-amber-bg)", text: <><strong>Booking Reminder</strong> — Study Room 4 starts in <strong>2 hours</strong></>,                          time: "2 hr ago",   unread: false },
